...
function renderUpgrades() {
  ...
  upgArea.style =
    "display: flex; flex-wrap: wrap; overflow-y: scroll; scrollbar-width: none;";
  wrapper.appendChild(upgArea);

  if (settlement.level < 4) {
    let disclaimer = document.createElement("div");
    disclaimer.style =
      "display: flex; align-items: center; justify-content: center; width: 100%; height: 50%;";
    disclaimer.textContent =
      "Your Settlement needs to be level 4 to unlock Settlement Upgrades";
  ...
  for (let i = 4; i <= settlement.level; i++) {
    ...
    row.style =
      "justify-content: space-around; width: 100%; margin-bottom: 1rem";
...
function showUpgradeDetails(level, id) {
  ...
  for (let i = 0; i < upg.content.length; i++) {
    ...
    text.style =
      'font-size: 18px; padding: 0 2rem; box-sizing: border-box; font-family: "monospace";';
