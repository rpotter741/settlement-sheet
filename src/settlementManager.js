...
import { settlementList } from "./storage";
...
function openSettlementSwap() {
    ...
    newBtn.textContent = 'New Settlement';
    newBtn.style = 'border-radius: 1rem;'
    ...
    displayArea.style = 'display: flex; flex-wrap: wrap;'

    Object.keys(settlementList).forEach((item) => {
        let btnBox = document.createElement('div');
        btnBox.style = 'display: flex; justify-content: center; align-items: center; width: 25%; padding: 1rem 0'

        let button = document.createElement('button');
        button.textContent = item;
        if(item == settlement.name) {
            button.style = 'border-radius: 1rem; background: green'
        } else {
            button.style = 'border-radius: 1rem;'
        }

        button.addEventListener('click', () => {
            let answer = confirm(`Do you want to switch to ${item}?`)
        ...
        displayArea.appendChild(btnBox);
    })
