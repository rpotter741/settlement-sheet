...
function safetyRating() {
  ...
  safety.Base = (
    ((a + b + c) / settlement.level) *
    (settlement.currentHealth /
      (settlement.maxHealthBase + settlement.maxHealthBonus))
  ).toFixed(1);
...
function diBox() {
  ...
  spBuy.textContent = "+";
  spBuy.style =
    "border-radius: 50%; font-size: 12px; align-self: end; margin-left: 4px";
  ...
  if (settlement.type == "" || settlement.type == "Survivalist") {
    toolTip.textContent =
      "Spend 2 Settlement Points to gain 1 Defensive Infrastructure";
  } else if (settlement.type == "Fortified") {
    toolTip.textContent =
      "Spend 1 Settlement Point to gain 1 Defensive Infrastructure";
  } else if (settlement.type == "Mercantile") {
    toolTip.textContent =
      "Spend 3 Settlement Points to gain 1 Defensive Infrastructure";
  }
...
let safActions = [
  {
    ...
    description:
      "A party member can attempt a Crafting or Athletics check to construct barricades in and around the settlement. This costs 5gp +2 per proficiency rank multiplied by the Settlement Level per use. This cannot increase Defensive Infrastructure above its max.",
  },
  {
    ...
    type: "defensive infrastructure",
    description:
      "A party member can attempt a Thievery or Crafting Check to create traps to protect from potential attackers. This costs 5gp +2 per proficiency rank multiplied by the Settlement Level per use. This cannot increase Defensive Infrastructure above its max.",
  },
  {
    name: "Erect Wards",
    id: "erectWards",
    type: "defensive infrastructure",
    description:
      "A party member can attempt an Arcane, Nature, Occult, or Religion check to erect magical alarms and traps to deter attackers or intruders. This cannot increase Defensive Infrastructure above its max.",
  },
  {
    name: "Scout Area",
    id: "scoutArea",
    type: "intelligence",
    description:
      "A party member can attempt a Nature, Perception, or Survival check to find potential dangers in the surrounding area.",
  },
  {
    ...
    type: "intelligence",
    description:
      "A party member can attempt a Society or Stealth check to pick up on rumors and scuttlebutt in the settlement, possibly learning of any insider threats.",
  },
  {
    name: "Train Citizens",
    id: "trainCitizens",
    type: "garrison",
    description:
      "A party member can train forces to be better suited for combat. This can only be done once per character level and requires you to be at least an expert in at least one weapon category. Make a Follow the Expert roll for the settlement, using its level and Safety Score divided by 5 as bonuses, plus 2/3/4 depending on your proficiency rating.",
  },
];
...
function showSafAction(item) {
  ...
  header.style =
    "display: flex; justify-content: center; width: 100%; padding: 1rem; background: rgba(200,200,230,.5); box-sizing: border-box; border-radius: .5rem .5rem 0 0;";
